...
import pandas as pd
import joblib
import time # Necesario para la app
import os
...
def entrenar(path_datos="datos_procesados.pkl", path_modelo="modelo_multinomial.pkl", path_img_cm="cm_multinomial.png"):
    """
    Ejecuta la lógica de entrenamiento del Naive Bayes Multinomial.
    ...
    # Adaptación de claves para compatibilidad
    ...
    # Entrenar modelo Modelo Multinomial
    modeloMultinomial = MultinomialNB()

    # --- LÓGICA ORIGINAL EXACTA ---
    # Aquí LIMITAR es False en tu script original
    LIMITAR = False
    if LIMITAR:
        N = 20000
    ...
    modeloMultinomial.fit(train_arr_cortado, train_y_cortado)

    print("Prediciendo...")
    pred = modeloMultinomial.predict(test_arr)
    ...
    # Llamada con tu color original
    matriz_confusion_guardar(test_y, pred, 'YlGnBu', path_img_cm)

    # Guardar modelo entrenado
    joblib.dump(modeloMultinomial, path_modelo)
    print(f"Modelo guardado en '{path_modelo}'")

    return modeloMultinomial, metrics_gui, pred
